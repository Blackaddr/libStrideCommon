...
public:
		...
		const double* const Params = NULL, const bool UsePower = false )
	{
		...
		setWindow( WinType, Params, UsePower, false, delay_s.FracDelay );
	}
	...
	{
		const double n = 1.0 - sqr( wn * Len2i + kaiser_s.KaiserLen2Frac );
		wn++;
		...
		return( besselI0( kaiser_s.KaiserBeta * sqrt( n )) * kaiser_s.KaiserMul );
	}
	...
	{
		const double f = exp( -0.5 * sqr( wn * gaussian_s.GaussianSigmaI +
			gaussian_s.GaussianSigmaFrac ));
	...
	void generateBand( double* op,
		CWindowFunc wfunc = &CDSPSincFilterGen :: calcWindowBlackman )
	{
		CSineGen f2( freq_s.Freq2, 0.0, 1.0 / R8B_PI );
		f2.generate();
		...
		if( freq_s.Freq1 < 0x1p-42 )
		{
			if( pw < 0.0 )
			{
				*op = freq_s.Freq2 * ( *this.*wfunc )() / R8B_PI;
			...
			{
				*op = freq_s.Freq2 * pow_a(( *this.*wfunc )(), pw ) / R8B_PI;
		...
		{
			CSineGen f1( freq_s.Freq1, 0.0, 1.0 / R8B_PI );
			f1.generate();

			if( pw < 0.0 )
			{
				*op = ( freq_s.Freq2 - freq_s.Freq1 ) * ( *this.*wfunc )() / R8B_PI;
		...
		const int opinc = 1 )
	{
		...
		const double pw = Power;
		const double fd = delay_s.FracDelay;
		int t = -fl2;
		...
		else
		{
			...
			t++;
			f = -f;
			const double ut = t + delay_s.FracDelay;
			*op = ( ut > Len2 ? 0.0 :
...
private:
		...
		const bool IsCentered )
	{
		...
		if( Params == NULL )
		{
			kaiser_s.KaiserBeta = 9.5945013206755156;
			Power = ( UsePower ? 1.9718457932433306 : -1.0 );
		}
		else
		{
			kaiser_s.KaiserBeta = clampr( Params[ 0 ], 1.0, 350.0 );
			Power = ( UsePower ? fabs( Params[ 1 ]) : -1.0 );
		}

		kaiser_s.KaiserMul = 1.0 / besselI0( kaiser_s.KaiserBeta );
		Len2i = 1.0 / Len2;
		kaiser_s.KaiserLen2Frac = delay_s.FracDelay * Len2i;
	}
		...
		const bool IsCentered )
	{
		...
		if( Params == NULL )
		{
			gaussian_s.GaussianSigmaI = 1.0;
			Power = -1.0;
		}
		else
		{
			gaussian_s.GaussianSigmaI = clampr( fabs( Params[ 0 ]), 1e-1, 100.0 );
			Power = ( UsePower ? fabs( Params[ 1 ]) : -1.0 );
		}

		gaussian_s.GaussianSigmaI *= Len2;
		gaussian_s.GaussianSigmaI = 1.0 / gaussian_s.GaussianSigmaI;
		gaussian_s.GaussianSigmaFrac = delay_s.FracDelay * gaussian_s.GaussianSigmaI;
	}
	...
	{
		delay_s.FracDelay = UseFracDelay;
		...
		if( WinType == wftCosine )
		{
			...
			else
			{
				...
				w1.init( step1, R8B_PId2 - step1 * fl2 + step1 * delay_s.FracDelay );

				const double step2 = R8B_2PI / Len2;
				w2.init( step2, R8B_PId2 - step2 * fl2 + step2 * delay_s.FracDelay );

				const double step3 = R8B_3PI / Len2;
				w3.init( step3, R8B_PId2 - step3 * fl2 + step3 * delay_s.FracDelay );
			}
